...
現行の15分固定モデルも別系統で6時間ごとに過去72時間を再検証します。実運用と同じ1取引5%・同時3件までの配分、Polymarket方向のみの同時対照、15分枠単位の信頼区間で評価し、最新結果と24回分の履歴をダッシュボードへ表示します。各実行はコード・仕様・入力データ・モデル判断を分離したSHA-256と市場単位の`observations.csv`を不変成果物として保存します。履歴足による結果は一次選別に限定し、実取引の許可には前向き5秒板監査50枠が必要です。
...
~/.polymarket-watch/hyperliquid-venv/bin/python scripts/provision-hyperliquid-testnet-wallet.py
```

プロビジョニングコマンドは専用APIウォレットを`~/.polymarket-watch/secrets/`へ権限600で保存し、公開してよいAPIウォレットアドレスだけを表示します。Hyperliquid testnetでこのアドレスをAPI Walletとして承認した後、`.env`へマスター口座の`HYPERLIQUID_ACCOUNT_ADDRESS`を設定します。鍵ファイルを変更する場合だけ`HYPERLIQUID_API_WALLET_KEY_FILE`を指定してください。平文の`HYPERLIQUID_API_WALLET_PRIVATE_KEY`も後方互換のため利用できますが推奨しません。

テストネット連動には、さらに`HYPERLIQUID_TESTNET_ENABLED=1`が必要です。最初は`HYPERLIQUID_TESTNET_AUTO_MIRROR=0`のまま、管理APIの`verification-suite`を実行します。これは成行の往復約定、板に残るALO注文、CLOID取消、接続し直し、緊急取消、最終残高照合を最大15 USDCで実行してDBへ証拠を保存します。実際の部分約定まで観測できない限り状態は`PARTIAL`のままで、画面はテストネット検証済みと表示しません。バックテストの品質判定が合格していない場合は、検証に成功しても自動ミラー注文は出ません。メインネット接続はコード上で無効です。

```sh
curl -X POST http://127.0.0.1:3001/api/hyperliquid-testnet \
  -H "authorization: Bearer $API_ACCESS_TOKEN" \
  -H "content-type: application/json" \
  -d '{"action":"verification-suite","asset":"BTC","notionalUsd":12,"confirmation":"TESTNET_ONLY"}'
```
...
日次バックアップはSQLiteのオンラインバックアップを作成し、AES-256で暗号化したあと、同じ鍵で別ファイルへ復号して`PRAGMA integrity_check`が`ok`になることまで確認します。SHA-256も確認記録へ残し、復元確認に失敗した世代は保存しません。サービス再起動時は前回成功時刻から残り時間を引き継ぎ、失敗時だけ5分後に再試行します。確認記録の期限切れも異常として扱います。
