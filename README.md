...
現行の15分固定モデルも別系統で6時間ごとに過去72時間を再検証します。実運用と同じ1取引5%・同時3件までの配分、Polymarket方向のみの同時対照、15分枠単位の信頼区間で評価し、最新結果と24回分の履歴をダッシュボードへ表示します。各実行はコード・仕様・データセットのSHA-256と市場単位の`observations.csv`を不変成果物として保存します。履歴足による結果は一次選別に限定し、実取引の許可には前向き5秒板監査50枠が必要です。
