...
- Polymarketの公式決着、エントリー時と終了時のHyperliquid 5秒板、エントリー時のPolymarket askが揃った取引だけを「完全監査済み」と数える
- 時刻ずれは15秒以内、Polymarketの板更新から保存までの経過は2分以内とする
- 欠測した取引を後から推定値で埋めず、欠測件数と監査率をそのまま残す
- 完全監査50件、コスト控除後プラス、対照戦略との差の95%信頼区間下限プラス、最大下落5%以内を満たすまで実取引を許可しない
