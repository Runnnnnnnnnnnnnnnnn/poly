...
- Chainlinkの開始・終了境界に最も近い実測値から `終了 >= 開始` を再計算し、Polymarketの正式決着と照合する。50市場、境界取得率95%以上、不一致0件、境界時刻誤差60秒以内を満たすまで決着整合ゲートを通さない
...
- 完全監査50件、監査率95%以上、同時対照の再現率95%以上、コスト控除後プラス、最良対照超過、対照差の95%下限プラス、試行補正後95%以上、最大下落5%以内、Chainlink方向と正式決着の一致をすべて満たすまで実取引を許可しない
