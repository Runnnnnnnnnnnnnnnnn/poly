...
- 売買判断は別々のHTTP取得値を合成せず、保存済みの同一5秒同期板からPolymarket確率、Hyperliquid価格、判定参照価格を読み出す。保存から判断までと各情報源の更新から保存までがすべて15秒以内の板だけを使う
- Polymarketの公式決着、注文時刻以後に保存された最初のエントリー・終了用Hyperliquid 5秒板、エントリー時のPolymarket askが揃った取引だけを「完全監査済み」と数える
- 15分市場は出来高順ではなく終了時刻の近い順で発見する。低出来高のETH・SOL・XRPを開始前から購読し、開始後に見つかった市場を推定で補完しない
- Polymarketの5分・15分市場用基準価格から `終了 >= 開始` を再計算し、正式決着と照合する。公開RTDSのChainlink価格は別系統の再構成監査に使う。50市場、公式価格取得率95%以上、正式決着との不一致0件、公開RTDS再構成との方向差0件を満たすまで決着整合ゲートを通さない
...
- 完全監査50件、監査率95%以上、同時対照の再現率95%以上、コスト控除後プラス、最良対照超過、対照差の95%下限プラス、試行補正後95%以上、最大下落5%以内、Polymarket基準価格・正式決着・公開RTDS再構成の一致をすべて満たすまで実取引を許可しない
