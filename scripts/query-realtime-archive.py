...
def main():
    ...
    for view, table in (("market_ticks", "realtime_market_tick"), ("asset_ticks", "realtime_asset_tick")):
        ...
        connection.execute(
            f"CREATE VIEW {view} AS SELECT * FROM read_parquet('{escaped_pattern}', hive_partitioning=true, union_by_name=true)"
        )
        registered.add(view)
    if "market_ticks" not in registered and not args.sql:
        ...
        return
    query = args.sql or """
        SELECT date, asset, count(*) AS rows, count(DISTINCT marketId) AS markets,
